...
class AuditLog(Base):
    ...
    Attributes:
        id (String): UUID 형식의 고유 식별자
        user_id (String): 행위를 수행한 사용자 ID (시스템 로그의 경우 Null)
        action (String): 수행된 행위 유형 (예: 로그인, 입금, 자문 요청)
        details (JSON): 행위에 대한 상세 정보
        ip_address (String): 행위 발생 IP 주소
        user_agent (String): 사용자 브라우저/클라이언트 정보
